...
import com.ync365.px.repository.TaskDao;
import com.ync365.px.repository.UserDao;
import com.ync365.px.service.ServiceException;
...
public class AccountService {
	...
	public void registerUserForSynchronizeData(User user) {
		entryptPasswordForSynchronizeData(user);
		user.setRoles("user");
		user.setRegisterDate(clock.getCurrentDate());
	 ...
	 * 设定安全的密码，生成随机的salt并经过1024次 sha-1 hash
	 */
	private void entryptPasswordForSynchronizeData(User user) {
		byte[] salt = Digests.generateSalt(SALT_SIZE);
		user.setSalt(Encodes.encodeHex(salt));
